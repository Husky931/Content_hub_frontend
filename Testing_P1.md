...
<!-- ## 17. Task Lifecycle Scenarios (10 Combinations)

> **Setup requirements:**
>
> - **Channels:** `#voiceover-basic` (requires tag `voiceover`), `#translation-cn` (requires tag `translation`), `#general-tasks` (no tag required)
> - **Users:** admin, mod1 (assigned to #voiceover-basic), mod2 (assigned to #translation-cn), supermod1, creator1 (has `voiceover` tag), creator2 (has `translation` tag), creator3 (has both tags)
> - **Rule:** The user who created a task CANNOT review submissions on it

### 17.1 Scenario: Basic submit → approve (single channel, single creator)

- [ ] 17.1.1 mod1 creates task "Record Greeting" in #voiceover-basic, publishes immediately
- [ ] 17.1.2 System message in #voiceover-basic shows: "mod1 posted a new task ..."
- [ ] 17.1.3 creator1 (has voiceover tag) sees the task in #voiceover-basic
- [ ] 17.1.4 creator2 (no voiceover tag) does NOT see #voiceover-basic or its tasks in /tasks
- [ ] 17.1.5 creator1 submits an attempt → sees "Submitted - pending review" with Edit/Delete
- [ ] 17.1.6 creator1 can no longer see "Submit Attempt" button (only Edit/Delete)
- [ ] 17.1.7 mod1 CANNOT review creator1's attempt (mod1 created the task)
- [ ] 17.1.8 supermod1 or admin reviews and approves → system message shows bounty
- [ ] 17.1.9 creator1 gets "Task approved!" notification → click navigates to #voiceover-basic
- [ ] 17.1.10 creator1 financials show increased balance

### 17.2 Scenario: Submit → reject → resubmit → approve

- [ ] 17.2.1 admin creates task "Translate Article" in #translation-cn, publishes
- [ ] 17.2.2 creator2 submits attempt with text
- [ ] 17.2.3 mod2 reviews and rejects with reason "Missing paragraph 3"
- [ ] 17.2.4 System message in channel shows: "...was rejected. Reason: Missing paragraph 3"
- [ ] 17.2.5 creator2 sees rejection notification with reason → click navigates to #translation-cn
- [ ] 17.2.6 creator2 sees "Rejected" badge on task card with "you can submit again"
- [ ] 17.2.7 creator2 submits a new attempt
- [ ] 17.2.8 supermod1 approves the second attempt → creator2 earns bounty

### 17.3 Scenario: Multiple creators submit, one approved (auto-reject others)

- [ ] 17.3.1 admin creates task in #general-tasks (no tag required), publishes
- [ ] 17.3.2 creator1, creator2, and creator3 all submit attempts
- [ ] 17.3.3 All three see "Submitted - pending review" on their respective cards
- [ ] 17.3.4 mod1 approves creator1's attempt
- [ ] 17.3.5 creator2 and creator3's attempts are auto-rejected with reason "Another attempt was approved"
- [ ] 17.3.6 Only creator1 gets the bounty (check financials)

### 17.4 Scenario: Edit and delete own submission

- [ ] 17.4.1 creator1 submits attempt on an active task
- [ ] 17.4.2 creator1 clicks Edit → textarea shows previous text, modifies it, saves
- [ ] 17.4.3 Verify updated text appears in the submitted view
- [ ] 17.4.4 creator1 clicks Delete → attempt is removed
- [ ] 17.4.5 creator1 sees "Submit Attempt" button again (can submit fresh)

### 17.5 Scenario: Mod/supermod/admin submits attempt on a task (non-creator roles)

- [ ] 17.5.1 admin creates task in #general-tasks
- [ ] 17.5.2 mod1 submits an attempt (mods can submit per permissions)
- [ ] 17.5.3 mod1 sees "Submitted - pending review" with Edit/Delete
- [ ] 17.5.4 supermod1 submits an attempt on the same task
- [ ] 17.5.5 admin CANNOT review (admin created the task)
- [ ] 17.5.6 mod2 reviews and approves mod1's attempt → mod1 earns bounty, supermod1 auto-rejected

### 17.6 Scenario: Task visibility — tag-gated channels

- [ ] 17.6.1 creator1 (voiceover tag only) opens /tasks → sees tasks from #voiceover-basic and #general-tasks
- [ ] 17.6.2 creator1 does NOT see tasks from #translation-cn
- [ ] 17.6.3 creator3 (both tags) opens /tasks → sees tasks from all three channels
- [ ] 17.6.4 supermod1 opens /tasks → sees all tasks (supermods bypass tag-gating)

### 17.7 Scenario: Notification navigation

- [ ] 17.7.1 creator1 submits attempt → mod1 gets "attempt_submitted" notification
- [ ] 17.7.2 mod1 clicks notification → navigates to /review?task=taskId
- [ ] 17.7.3 mod2 approves → creator1 gets "task_approved" notification
- [ ] 17.7.4 creator1 clicks notification → navigates to the task's channel page
- [ ] 17.7.5 After rejection: creator1 clicks "task_rejected" notification → navigates to channel
- [ ] 17.7.6 After payout: creator1 clicks "payout" notification → navigates to /financials

### 17.8 Scenario: Self-review prevention

- [ ] 17.8.1 mod1 creates a task and creator1 submits
- [ ] 17.8.2 mod1 opens /review → does NOT see creator1's submission (because mod1 created the task)
- [ ] 17.8.3 mod1 tries to review via API (PATCH) → gets 403 "You cannot review submissions on a task you created"
- [ ] 17.8.4 supermod1 opens /review → DOES see the submission and can approve/reject -->
